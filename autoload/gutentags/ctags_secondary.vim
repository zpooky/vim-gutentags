...
let g:gutentags_ctags_extra_args = get(g:, 'gutentags_ctags_extra_args_secondary', [])
...
function! gutentags#ctags_secondary#init(project_root) abort
    ...
    let b:gutentags_files['ctags_secondary'] = gutentags#get_cachefile(
                \a:project_root, l:tagfile).'_secondary' " SPOOKY this is shared with gutentags.vim

    " Set the tags file for Vim to use.
    if g:gutentags_ctags_auto_set_tags
        " set tags=...
        if has('win32') || has('win64')
            execute 'setlocal tags+=' . fnameescape(b:gutentags_files['ctags_secondary'])
        else
            " spaces must be literally escaped in tags path
            let l:literal_space_escaped = substitute(fnameescape(b:gutentags_files['ctags_secondary']), '\ ', '\\\\ ', 'g')
...
" SPOOKY I guess this generates the argument list for ./update_scopedb.sh -L g:gutentags_file_list_command ...
function! gutentags#ctags_secondary#generate(proj_dir, tags_file, gen_opts) abort
    ...
    if l:write_mode == 0 && l:tags_file_exists
        ...
        let l:file_list_cmd = gutentags#get_project_file_list_cmd_secondary()
    ...
    if !g:gutentags_fake
        let l:job_opts = gutentags#build_default_job_options('ctags_secondary')
        let l:job = gutentags#start_job(l:cmd, l:job_opts)
        call gutentags#add_job('ctags_secondary', a:tags_file, l:job)
    else
        call gutentags#trace("(fake... not actually running)")
    endif
endfunction

function! gutentags#ctags_secondary#on_job_exit(job, exit_val) abort
    call gutentags#remove_job_by_data('ctags_secondary', a:job)
